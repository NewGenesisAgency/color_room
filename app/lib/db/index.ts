...
function seedSimpleClickGame(db: Database.Database) {
  ...
  const config = {
    ...
    icon: 'Zap',
    difficulty: 1,
    description: 'La dalle 1 s\'allume (ou pas) au hasard. Clique-la : si elle est allumée → son de réussite + points, sinon → son d\'échec.',
    // Démarrage : on tire au sort si la dalle 1 (tileIndex 0) s'allume, puis on
    // l'allume SEULE (tile_set) ou on l'éteint. Au clic SUR la dalle 1 précisément
    // (on_tile_click {tileIndex:0}), on compare : allumée = succès, sinon échec.
    nodes: [
      { id: 'n_begin',  kind: 'event_begin',    name: 'Démarrer',            enabled: true, params: {}, pos: { x: 60, y: 80 } },
      { id: 'n_score',  kind: 'score_reset',    name: 'Score 0',             enabled: true, params: {}, pos: { x: 320, y: 80 } },
      { id: 'n_rand',   kind: 'random_int',     name: 'Aléa 0 ou 1',         enabled: true, params: { min: 0, max: 1, varName: 'light_on' }, pos: { x: 580, y: 80 } },
      { id: 'n_ifon',   kind: 'if',             name: 'Dalle 1 allumée ?',   enabled: true, params: { varName: 'light_on', op: 'eq', value: 1 }, pos: { x: 840, y: 80 } },
      { id: 'n_on',     kind: 'tile_set',       name: 'Allumer dalle 1',     enabled: true, params: { tileIndex: 0, color: '#f97316', intensity: 0.9 }, pos: { x: 1100, y: 0 } },
      { id: 'n_off',    kind: 'tile_set',       name: 'Éteindre dalle 1',    enabled: true, params: { tileIndex: 0, color: '#0a0e1a', intensity: 0 }, pos: { x: 1100, y: 220 } },
      { id: 'n_click',  kind: 'on_tile_click',  name: 'Clic sur dalle 1',    enabled: true, params: { tileIndex: 0 }, pos: { x: 60, y: 400 } },
      { id: 'n_ifcor',  kind: 'if',             name: 'Était allumée ?',     enabled: true, params: { varName: 'light_on', op: 'eq', value: 1 }, pos: { x: 320, y: 400 } },
      { id: 'n_snd_ok', kind: 'play_sound',     name: 'Son réussite',        enabled: true, params: { sound: 'success' }, pos: { x: 580, y: 300 } },
      { id: 'n_add',    kind: 'add_score',      name: 'Plus 10 pts',         enabled: true, params: { amount: 10 }, pos: { x: 840, y: 300 } },
      { id: 'n_snd_ko', kind: 'play_sound',     name: 'Son échec',           enabled: true, params: { sound: 'error' }, pos: { x: 580, y: 500 } },
    ],
    edges: [
      { id: 'e1', from: 'n_begin', to: 'n_score' },
      { id: 'e2', from: 'n_score', to: 'n_rand' },
      { id: 'e3', from: 'n_rand', to: 'n_ifon' },
      // if : 1re sortie = vrai (allumer), 2e = faux (éteindre)
      { id: 'e4', from: 'n_ifon', to: 'n_on' },
      { id: 'e5', from: 'n_ifon', to: 'n_off' },
      // Au clic sur la dalle 1 : succès si elle était allumée, sinon échec
    ...
    uiLayout: [
      { id: 'u_title', kind: 'title_banner', x: 20, y: 10, width: 400, height: 50, text: 'Test Simple - Dalle 1', bgColor: '#0a0e1a', textColor: '#f97316', fontSize: 18 },
      { id: 'u_score', kind: 'score_display', x: 20, y: 80, width: 180, height: 80, text: 'Score', varBind: 'score' },
      { id: 'u_msg', kind: 'message_box', x: 220, y: 80, width: 260, height: 80, bgColor: 'rgba(249,115,22,0.2)', text: 'Clique la dalle 1 SI elle est allumée en orange → réussite. Sinon → échec.' },
