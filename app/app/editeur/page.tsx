...
export default function EditeurPage() {
  ...
  useEffect(() => {
    ...
    function executeNodeSync(nodeId: string, depth = 0): void {
      ...
      switch (node.kind) {
        ...
        case 'tile':
        case 'tile_set': {
          // tileIndex : nombre OU nom de variable (robuste si l'IA met une
          // variable dans tile_set au lieu de tile_set_var).
          const rawIdx = node.params.tileIndex;
          let idxNum: number;
          if (typeof rawIdx === 'number') idxNum = rawIdx;
          else {
            const s = String(rawIdx ?? '0').trim();
            idxNum = /^[-+]?\d+$/.test(s) ? Number(s) : Number(vars[s] ?? 0);
          }
          const tileIndex = Math.max(0, Math.min(tiles.length - 1, Math.round(Number.isFinite(idxNum) ? idxNum : 0)));
          const color = getColor(node.params, 'color', '#ff2aa6');
          const intensity = clamp01(getNum(node.params, 'intensity', 0.85));
          tiles[tileIndex] = { color, intensity };
          break;
  ...
  const sendAiMessage = async (override?: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data?.ok) {
        // Message affiché UNE seule fois (dans le fil de discussion). Les refus
        // hors-périmètre (OUT_OF_SCOPE) sont formulés tels quels, sans préfixe
        // "Erreur :" ; les vraies erreurs gardent le préfixe.
        const isScope = data?.error === 'OUT_OF_SCOPE';
        const msg = data?.message || 'Échec de la génération.';
        setAiMessages([...baseMsgs, { id: aiMkId(), role: 'assistant', content: isScope ? msg : 'Erreur : ' + msg, ts: Date.now(), error: true }]);
        return;
      }
            ...
            {aiMessages.length === 0 && !aiBusy && (
              <div style={{ margin: 'auto', textAlign: 'center', color: 'rgba(15,23,42,0.55)', fontSize: 13, maxWidth: 340 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    { t: 'Color Speed (vert/rouge, difficulté réglable)', p: "Crée \"Color Speed\", un jeu de réflexes éducatif. Le jeu DÉMARRE DIRECTEMENT sur le bloc Démarrer (event_begin) — PAS d'écran de configuration ni de bouton Démarrer à cliquer. Dès le départ : score à 0, difficulté par défaut moyenne (variable difficulty=2), puis à chaque manche allume EN MÊME TEMPS deux dalles aléatoires distinctes (random_int dans des variables + tile_set_var), une VERTE (#22c55e, la bonne) et une ROUGE (#ef4444, le piège) — garantis qu'elles sont différentes. Il faut cliquer la dalle VERTE le plus vite possible : les points gagnés vont jusqu'à 50 et diminuent selon le nombre de secondes écoulées. Si on clique la ROUGE, on perd des points (malus = difficulty×5). Joue \"correct\" sur la verte, \"wrong\" + vibration sur la rouge, puis re-tire deux nouvelles dalles. Optionnel : 2-3 boutons de difficulté (1/2/3) qui changent la variable difficulty EN DIRECT sans arrêter le jeu. Affiche score et minuteur. NE mets PAS de grille de dalles dans l'interface : la salle est déjà visible en 3D." },
                    { t: 'Morpion mural (duo / multi / IA minimax)', p: "Crée un Morpion (tic-tac-toe) qui se joue sur 9 dalles formant un carré 3×3 sur le mur du fond de la salle gauche (et le même 3×3 dans la salle droite en miroir). Le jeu DÉMARRE DIRECTEMENT sur le bloc Démarrer (event_begin) — PAS de bouton Démarrer à cliquer : une partie en mode Duo commence aussitôt (joueur 1 bleu, joueur 2 rouge, à tour de rôle sur le même appareil, on alterne le joueur courant à chaque coup). Propose en plus, via des boutons UI optionnels, de basculer le mode EN DIRECT : \"Multijoueur\" (réseau via mp_session) ou \"IA\" (contre l'ordinateur, un bloc script_python qui implémente un minimax dont la force dépend d'une variable difficulty 1/2/3, comme l'IA du Puissance 4 — difficulté 3 = imbattable). Représente le plateau avec une grille logique synchronisée vers les dalles, détecte les 3 alignées pour la victoire, joue \"win\"/\"lose\". Interface : titre, mode courant, joueur courant, score - mais AUCUNE représentation des dalles (la 3D est déjà à l'écran)." },
                    { t: 'Simon (mémoire de séquence, difficulté)', p: "Crée un \"Simon\" de mémoire, jeu éducatif d'attention et de mémorisation. Utilise 4 zones de couleur fixes sur 4 dalles distinctes (par ex. les 4 coins) : ROUGE (#ef4444), VERTE (#22c55e), BLEUE (#3b82f6) et JAUNE (#eab308), chacune avec un son différent. Le jeu DÉMARRE DIRECTEMENT sur le bloc Démarrer (event_begin) — PAS d'écran de config ni de bouton Démarrer : la première séquence se lance aussitôt avec une difficulté par défaut (variable difficulty=2 qui règle la vitesse d'affichage) ; des boutons 1/2/3 optionnels peuvent changer difficulty EN DIRECT. Déroulé d'une manche : 1) phase MONTRER — le jeu rejoue une séquence de couleurs (au début 1, puis +1 couleur par manche), en allumant chaque dalle ~400 ms avec son son ; 2) phase REPRODUIRE — le joueur reclique les dalles dans le MÊME ordre. Bonne entrée → son \"correct\" + la dalle s'allume brièvement + petits points ; séquence complète → son \"win\", bonus de score croissant avec la manche, et la séquence s'allonge. Une SEULE erreur (mauvaise dalle ou mauvais ordre) → son \"wrong\" + flash rouge + Game Over avec le score final. Affiche le numéro de manche et le score. Empêche tout clic pendant la phase MONTRER. NE mets AUCUNE grille ni représentation des dalles dans l'interface : la salle 3D est déjà affichée, le joueur clique directement les dalles." },
                  ].map((ex) => (
