            ...
            {aiMessages.length === 0 && !aiBusy && (
              <div style={{ margin: 'auto', textAlign: 'center', color: 'rgba(15,23,42,0.55)', fontSize: 13, maxWidth: 340 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {[
                    { t: 'Color Speed (vert/rouge, difficulté réglable)', p: "Crée \"Color Speed\", un jeu de réflexes éducatif. Écran de configuration au départ (titre + boutons) : le joueur choisit s'il joue sur 1 salle (dalles 0 à 20) ou 2 salles (toutes les dalles), et une difficulté 1/2/3 qui règle la vitesse. Quand il appuie sur Démarrer : à chaque manche, allume EN MÊME TEMPS deux dalles aléatoires distinctes, une VERTE (#22c55e, la bonne) et une ROUGE (#ef4444, le piège). Il faut cliquer la dalle VERTE le plus vite possible : les points gagnés vont jusqu'à 50 et diminuent selon le nombre de secondes écoulées depuis l'apparition. Si on clique la ROUGE, on perd des points (le malus augmente avec la difficulté). Joue \"correct\" sur la verte, \"wrong\" + vibration sur la rouge. Affiche score et minuteur. NE mets PAS de grille de dalles dans l'interface : la salle est déjà visible en 3D." },
                    { t: 'Morpion mural (duo / multi / IA minimax)', p: "Crée un Morpion (tic-tac-toe) qui se joue sur 9 dalles formant un carré 3×3 sur le mur du fond de la salle gauche (et le même 3×3 dans la salle droite en miroir). Écran de config au départ avec 3 modes : \"Duo\" (deux joueurs à tour de rôle sur le même appareil, on alterne le joueur courant à chaque coup), \"Multijoueur\" (en réseau via mp_session), et \"IA\" (contre l'ordinateur). En mode IA, l'adversaire est un bloc script_python qui implémente un minimax dont la force dépend d'une difficulté 1/2/3 réglable, exactement comme l'IA du Puissance 4 (difficulté 3 = imbattable). Représente le plateau avec une grille logique synchronisée vers les dalles, alterne les symboles en couleur (joueur 1 bleu, joueur 2 rouge), détecte les 3 alignées pour la victoire, joue \"win\"/\"lose\". Interface : titre, mode, joueur courant, score - mais AUCUNE représentation des dalles (la 3D est déjà à l'écran)." },
                    { t: 'Simon (mémoire de séquence, difficulté)', p: "Crée un \"Simon\" de mémoire, jeu éducatif d'attention et de mémorisation. Utilise 4 zones de couleur fixes sur 4 dalles distinctes (par ex. les 4 coins) : ROUGE (#ef4444), VERTE (#22c55e), BLEUE (#3b82f6) et JAUNE (#eab308), chacune avec un son différent. Écran de configuration au départ (titre + boutons) pour choisir une difficulté 1/2/3 qui règle la vitesse d'affichage de la séquence ; un bouton Démarrer lance la partie. Déroulé d'une manche : 1) phase MONTRER — le jeu rejoue une séquence de couleurs (au début 1, puis +1 couleur par manche), en allumant chaque dalle ~400 ms avec son son ; 2) phase REPRODUIRE — le joueur reclique les dalles dans le MÊME ordre. Bonne entrée → son \"correct\" + la dalle s'allume brièvement + petits points ; séquence complète → son \"win\", bonus de score croissant avec la manche, et la séquence s'allonge. Une SEULE erreur (mauvaise dalle ou mauvais ordre) → son \"wrong\" + flash rouge + Game Over avec le score final. Affiche le numéro de manche et le score. Empêche tout clic pendant la phase MONTRER. NE mets AUCUNE grille ni représentation des dalles dans l'interface : la salle 3D est déjà affichée, le joueur clique directement les dalles." },
                  ].map((ex) => (
