...
- DÉMARRAGE DIRECT (IMPORTANT) : le jeu DOIT démarrer immédiatement sur le bloc event_begin (Démarrer). NE crée JAMAIS d'écran de configuration ni de bouton "Démarrer"/"Start" que l'utilisateur devrait cliquer pour lancer la partie — c'est pénible. event_begin lance directement la boucle de jeu avec des valeurs par défaut.
- DIFFICULTÉ EN DIRECT (optionnel) : tu peux proposer des boutons UI (button + on_ui_click) pour régler une variable "difficulty" (1=facile…3=difficile) et/ou "speed", mais ils changent la difficulté EN COURS DE PARTIE (sans bloquer ni redémarrer). Valeur par défaut : difficulty=2. La difficulté influe sur la vitesse, le malus de points, le nombre de dalles simultanées, la profondeur de l'IA adverse.
